...
/// `.off` always uses percentile windowing (legacy behaviour).
/// `.auto` (default) detects label volumes and colours them: canonical FreeSurfer
/// colours when the labels look like a FreeSurfer parcellation, otherwise
/// deterministic random colours per label.
